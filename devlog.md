# 手写一个多 agent 编排器内核,结果发现难的根本不是编排
...
下面每一节基本都对应仓库里一个能跑的脚本(`bin/try-*`、`bin/eval-*`)。别信我说的,自己跑——这本来就是全文的主题。
...
那条"井字棋容易漏判平局",我没让它白翻,存进了一个跨运行的记忆。下次再做井字棋,编排器先把相关经验捞出来塞给 worker:
...
第二句其实是第一句的方法:别信声称,信探针。能力表说支持 schema,真打 API 才知道被拒;Critic 看着在干活,建了 eval 才发现它幻觉又漏判;路由器规则版看着够用,量一下才知道换个说法就漏:
...
仓库里每个概念都配了一个你能自己跑、自己验的脚本,代码地图在 [README](./README.md)。点 "Open in Codespaces" 就能在云端真终端里跑(自带一把 DeepSeek key 即可)。别信上面的数字,自己跑一遍。
